  ...
  return (
    <div className="space-y-2">
      ...
      <div className="space-y-1 overflow-hidden">
        {error && (
          <p style={{ animation: "fadeIn 0.2s ease-out" }} className="rounded-lg border border-red-500/10 bg-red-950/40 px-3 py-2 text-xs text-red-400">
            {error}
          </p>
        )}

        {duplicateMsg && (
          <p style={{ animation: "fadeIn 0.2s ease-out" }} className="rounded-lg border border-amber-500/10 bg-amber-950/30 px-3 py-2 text-xs text-amber-400">
            {duplicateMsg}
          </p>
        )}

        {crossRefMsg && (
          <p style={{ animation: "fadeIn 0.2s ease-out" }} className="rounded-lg border border-sky-500/10 bg-sky-950/30 px-3 py-2 text-xs text-sky-400">
            {crossRefMsg}
          </p>
        )}
      ...
      {results.length > 0 && (
        <ul className="max-h-48 space-y-1 overflow-y-auto rounded-xl border border-white/[0.06] bg-white/[0.02] p-1.5 backdrop-blur-xl">
          {results.map((s, i) => (
            <li
              key={s.id}
              style={{ animation: `slideUp 0.2s ease-out ${i * 30}ms both` }}
            >
              <button
